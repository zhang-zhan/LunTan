...
from downPageLinYi import *


class Down_LinYi_Forum():
    ...
    def start_load(self, p_url, reList, first):
        ...
        try:
            ...
            next_page = next_page.split("http://bbs.langya.cn/")[1]
        except:
            next_page = None
        print next_page
        ...
        for htstr in mydic:
            try:
                ...
                try:
                    ...
                    rt = soup2.find("th",class_="new").find_all("span")[-1]["title"]
                    timeArray = time.strptime(rt, "%Y-%m-%d %H:%M")
                except:
                    rt = soup2.find("th",class_="new").find_all("a")[-1].get_text()
                    timeArray = time.strptime(rt, "%Y-%m-%d %H:%M")
                # try:
                ...
                #     timeArray = time.strptime(rt, "%Y-%m-%d %H:%M")
                print rt
                purl = soup2.find('th',class_="new").find('span',class_="xst").find_all("a")[-1]["href"]
                print purl
                nt =  time.time()
...
def scanItemUrl(url, initDeadline = 1414640120):
    ...
    downlist = Down_LinYi_Forum(turl, deadline)
    ...
    for myurl in urlList2:
        myurl = myurl.split("http://bbs.langya.cn/")[1]
        print 'L:%s' % myurl
...
def scanItemRun():
    #天涯的版块URL
    section = ["f-113-1.html",'f-2-1.html',"f-306-1.html","f-46-1.html","f-178-1.html"]
