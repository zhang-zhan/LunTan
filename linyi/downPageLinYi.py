...
import time
class Down_LinYi_Post(threading.Thread):
    ...
    def run(self):
        ...
        for num in range(self.startnum, self.endnum):
            ...
            self.pageurl = urlsplit[0]+"-"+str(num)+"-"+urlsplit[-1]
            self.num = num
    ...
    def down_text(self, last_page):
        ...
        soup = BeautifulSoup(html_content,from_encoding="gbk")
        #对于帖子不存在了的情况进行判断
        ...
        if((self.num == 1) and (self.rc == 0)):
            try:                    #帖子不存在是返回空
                title = soup.find('div', id='pt').find("div",class_="z").find_all("a")[-1].get_text()
            except:
                return []

            #获取板块
            section = soup.find("div",id="pt").find('div', class_='z').find_all('a')[-2].get_text()
            auname = soup.find('div', class_='pi').find('div',class_="authi").find('a').get_text()
            auid = soup.find('div', class_='pi').find('div',class_="authi").find('a')["href"].split("=")[-1]
            try:
                ctime = soup.find_all("div",class_="authi")[1].find("em").get_text()[4:].encode("utf-8")
        ...
        for htstr in mydic:
            ...
            if((mi == 0) and (self.num == 1)):
                ...
                if(self.rc == 0):
                    ...
                    imgsrc =""
                    try:
                        imgFile= soup2.find('td', class_='plc').find("td",class_="t_f").find_all("ignore_js_op")
                        for img in imgFile:
                    ...
                    infoText = ""
                    try:
                        oinfoText = soup2.find('td', class_='plc').find("td",class_="t_f").find_all("ignore_js_op")
                    ...
                    attachText = None
                    try:
                        attachText = soup2.find("td",class_="plc").find("td",class_="t_f").find("div",class_="attach_nopermission attach_tips").get_text()
                        strtext = strtext.replace(attachText,"")
                    except:
                        pass
                    psText = None
                    try:
                        psText = soup2.find("td",class_="plc").find("td",class_="t_f").find("i",class_="pstatus").get_text()
                        strtext = strtext.replace(psText,"")
                    except:
                        pass
            ...
            try:
                ...
                try:
                    rtime = soup2.find('td', class_='plc').find("div",class_="authi").find("em").get_text()[4:]
                    timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M:%S")
                except Exception as e:
                    try:
                        rtime = soup2.find('td', class_='plc').find("div",class_="authi").find("span")["title"]
                        timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M:%S")
                    except:
                        rtime = soup2.find('td', class_='plc').find("div",class_="authi").find_all("span")[1]["title"]
                        timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M:%S")
                ...
                oinfo = ""
                try:
                    oinfo = soup2.find("td",class_="plc").find('td', class_="t_f").find_all("ignore_js_op")
                    for info in oinfo:
                        strtext = strtext.replace(info.get_text(),"")
                except:
                    pass
                ...
                imgsrc =""
                try:
                    imginfo = soup2.find("td",class_="plc").find('td', class_="t_f").find_all("ignore_js_op")
                    for img in imginfo:
                        for im in img.find_all("img"):
                            try:
                                imgsrc += im["file"] + "\n"
                ...
                uid = soup2.find('td', class_='pls').find('div',class_="authi").find("a")["href"].split("=")[-1]
...
def page(url):
    ...
    while data>0:
        ...
        try:
            ...
            totalpage = soup.find("div",id="pgt").find("div",class_="pg").find("span")["title"].split()[1]
            break
        except Exception as e:
...
def down_page_run(urlstr):
    ...
    #设置线程数量
    thread_num = 4
    url = 'http://bbs.langya.cn/'+ urlstr
    """print 'URL:%s' % url"""
    patterMatch = re.compile(r"t(\d+)")
    idResult = patterMatch.search(urlstr)
    postid = idResult.group(1)
    #需要自己实现
    ...
    for num in range(0, thread_num-1):
        downlist = Down_LinYi_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_LinYi_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
def insert_log(record):
    ...
    if os.path.exists("./log/") is False:
        os.mkdir("./log/")
    with open('log/downDongYingData.log', 'a') as log:
...
if __name__ == '__main__':
    ...
    down_page_run("t315941-1-25.html")
