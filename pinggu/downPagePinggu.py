...
import time
class Down_PingGu_Post(threading.Thread):
        ...
        for htstr in mydic:
            ...
            try:
                ...
                try:
                    ...
                    rtime = soup2.find('td', class_="plc").find('div',class_="authi").find("em").find("span")["title"]
                timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M:%S")
...
def down_page_run(urlstr):
    ...
    for num in range(0, thread_num-1):
        downlist = Down_PingGu_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_PingGu_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
def insert_log(record):
    ...
    if os.path.exists("./log/") is False:
        os.mkdir("./log/")
    with open('log/downPingGuData.log', 'a') as log:
...
if __name__ == '__main__':
    ...
    down_page_run("thread-559013-1-7.html")
