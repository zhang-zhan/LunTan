...
import time
class Down_DouBan_Post(threading.Thread):
    ...
    """
    def __init__(self, postid, url, startnum,endnum, lastpage, replycount,resultdic,sec):
    ...
    def run(self):
        ...
        for num in range(self.startnum, self.endnum):
            #根据格式调整url
            if num !=1:
                self.pageurl = self.url+"?start="+ str((num-1)*100)
            self.num = num
    ...
    def down_text(self, last_page):
        ...
        if((self.num == 1) and (self.rc == 0)):
            title = soup.find('div', id="content").find("h1").get_text().strip("\r\t\n")
            #section = soup.find('p', class_='crumbs').find_all('a')[1].get_text()
            auname = soup.find('div', id='content').find('span',class_="from").find('a').get_text().strip("\r\t\n")
...
def page(url):
    ...
    try:
        ...
        page_result = html_content.find("span",class_="thispage")["data-total-page"]
    except:
        return None
    if page_result:
...
def getUpdateInfo(postid):
    ...
    try:
        ...
        pass
    return {'pg':1, 'rc':0}
def down_page_run(urlstr,sec="culture"):
...
def insert_log(record):
    ...
    if os.path.exists("./log") is False:
        os.mkdir("./log")
    with open('log/downDbData.log', 'a') as log:
...
if __name__ == '__main__':
    ...
    down_page_run("http://www.douban.com/group/topic/36989753/")
