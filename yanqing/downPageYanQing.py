...
class Down_YanQing_Post(threading.Thread):
    ...
    def down_text(self, last_page):
        ...
        if((self.num == 1) and (self.rc == 0)):
            ...
            try:
                auname = soup.find('div', class_='pi').find('div',class_="authi").find('a').get_text().strip("\r\t\n")
            except:
        ...
        for htstr in mydic:
            ...
            try:
                ...
                if(self.uid == uid):
                    ...
                    redic['au'] = False
                rtime = soup2.find('td', class_="plc comiis_vtx").find('div',class_="authi").find("em").get_text()[4:]
...
def down_page_run(urlstr):
    ...
    for num in range(0, thread_num-1):
        downlist = Down_YanQing_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_YanQing_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
if __name__ == '__main__':
    ...
    down_page_run("thread-25626-1-7.html")
