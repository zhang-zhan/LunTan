...
import time
import sys
from downPagemiyun import *
from bs4 import BeautifulSoup
reload(sys)
sys.setdefaultencoding('utf8')
class Down_MiYun_Forum():
    ...
    def start_load(self, p_url, reList, first):
        ...
        import re
        mydic = soup.find_all("tbody",id=re.compile(r"normalthread_(\d+)"))
        next_page = None
        next_page = soup.find('a', class_="nxt")["href"]
        next_page = next_page.split("http://bbs.miyun360.com")[1]
        print next_page
        #获取每个帖子的url
        timestamp = 0
        for htstr in mydic:
...
def scanItemUrl(url, initDeadline = 1414640120):
    ...
    downlist = Down_MiYun_Forum(turl, deadline)
