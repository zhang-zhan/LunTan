...
import time
class Down_HuaiRou_Post(threading.Thread):
    ...
    def down_text(self, last_page):
        ...
        if((self.num == 1) and (self.rc == 0)):
            title = soup.find('div', id='postlist').find("h2",class_="z").find("span").get_text()
            ...
            auid = soup.find('div', class_='pi').find('div',class_="authi").find('a')["href"].split("-")[-1].split(".")[0]


            try:
                ctime = soup.find('td', class_='plc').find("div",class_="authi").find("em").get_text()[4:]
                timeArray = time.strptime(ctime, "%Y-%m-%d %H:%M:%S")
            except:
                ctime = soup.find('td', class_='plc').find("div",class_="authi").find("span")["title"]
                timeArray = time.strptime(ctime, "%Y-%m-%d %H:%M:%S")
        ...
        mydic = soup.find('div',attrs={"id":"ct"}).find_all("div",id=re.compile(r"post_(\d+)"))
        mi = 0
        for htstr in mydic:
            ...
            if((mi == 0) and (self.num == 1)):
                ...
                if(self.rc == 0):
                    ...
                    try:
                        ...
                        pass
                    try :
                        strtext = strtext.replace(soup2.find('td', class_='plc').find("td",class_="t_f").find("div",class_="a_pr").get_text(),"")
                    except:
                        pass
                    try:
                        strtext = strtext.replace(soup2.find('td', class_='plc').find("td",class_="t_f").find("div",class_="attach_nopermission attach_tips").get_text(),"")
                    except:
                        pass
            ...
            try:
                ...
                try:
                    rtime = soup.find('td', class_='plc').find("div",class_="authi").find("em").get_text()[4:]
                    timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M:%S")
                except:
                    rtime = soup.find('td', class_='plc').find("div",class_="authi").find("span")["title"]
                    timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M:%S")
                ...
                oinfo = ""
                try:
                    oinfo = soup2.find("td","plc").find('td', class_="t_f").find_all("ignore_js_op")
                    for info in oinfo:
                        strtext = strtext.replace(info.get_text(),"")
                except:
                    pass
                ...
                imgsrc =""
                try:
                    imginfo = soup2.find("td","plc").find('td', class_="t_f").find_all("ignore_js_op")
                    for img in imginfo:
                        for im in img.find_all("img"):
                            try:
                                imgsrc += im["file"] + "\n"
...
def page(url):
    ...
    while data>0:
        ...
        try:
            ...
            totalpage = soup.find("div",class_="pg").find("span")["title"].split()[1]
            break
        except Exception as e:
...
def down_page_run(urlstr):
    ...
    for num in range(0, thread_num-1):
        downlist = Down_HuaiRou_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_HuaiRou_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
def insert_log(record):
    ...
    if os.path.exists("./log/") is False:
        os.mkdir("./log/")
    with open('log/downNchuanData.log', 'a') as log:
...
if __name__ == '__main__':
    ...
    down_page_run("http://www.hrlife.net/thread-221019-1-1.html")
