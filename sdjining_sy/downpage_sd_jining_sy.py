...
class Down_sdjnsy_Page(threading.Thread):
    ...
    def down_text(self, last_page):
        ...
        if ((self.num == 1) and (self.rc == 0)):
            ...
            title=title_section.split('_')[0]
            #print title
            section=title_section.split('_')[1].strip('[').strip(']')
            #print section
            # 获取作者名字
...
def down_page_run(urlstr):
    ...
    for num in range(0, thread_num - 1):
        downlist = Down_sdjnsy_Page(postid, url, num * numperthread + oldpage, (num + 1) * numperthread + oldpage,
                                  my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_sdjnsy_Page(postid, url, numperthread * (thread_num - 1) + oldpage, my_page + 1, my_page, oldrc,
                              resultdic )
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
...
if __name__ == '__main__':
    ...
    down_page_run("thread-989729-1-1.html")
