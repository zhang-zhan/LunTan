...
import time
import sys
import downpage_sd_jining_sy
import re
from bs4 import BeautifulSoup


class Down_sd_jining_sy_Forum():
    ...
    def start_load(self, p_url, reList, first):
        ...
        next_page = None
        next_page = soup.find('div', class_='pg').find('a',class_='nxt')['href'].split('/')[3]
        #print next_page
        #获取每个帖子的url
        timestamp = 0
        #获取版块主题的帖子
        for htstr in mydic:
            try:
                ...
                purl = soup2.find('a',class_='s xst')['href'].split('/')[3]
...
def scanItemUrl(url, initDeadline = 1414640120):
    ...
    downlist = Down_sd_jining_sy_Forum(turl, deadline)
	...
	downpage_sd_jining_sy.down_page_run(myurl)
...
def scanItemRun():
    #论坛的版块URL
    section = ['forum-21-1.html','forum-4-1.html']
