...
class Down_sdjnxs_Page(threading.Thread):
    ...
    def run(self):
        ...
        for num in range(self.startnum, self.endnum):
            ...
            if (num == self.lastpage):
                restmp = self.down_text(True,num)
                ...
                restmp = self.down_text(False,num)
                if restmp is False:
                    continue
                if len(restmp) is not 0:
                    self.resultdic[num] = restmp
    ...
    # 以下以天涯论坛为例，需要自己修改
    def down_text(self, last_page,nu):
        ...
        for htstr in mydic:
                try:
                    ...
                    if ((mi == 0) and (self.num == 1)):
                        ...
                        if(self.rc==0):
                            # 回帖的内容
                            if soup2.find('div',class_='t_fsz'):
                                strtext=soup2.find('div',class_='t_fsz')
                            else:
                            ...
                            #print strtext1
                            if strtext.find('div',class_='quote'):
                            ...
                            # 对视频的判断
                            if strtext.find_all('script'):
                                for script in strtext.find_all('script'):
                                    temp=script.get_text().strip()
                                    ps=re.compile(r"http://\S*.swf")
                                    pr=ps.search(temp)
                                    #print pr.group(0)
                                    if pr:
                                        strtext1=strtext1.replace(temp,pr.group(0)+'\n')
                                    else:
                                        strtext1=strtext1.replace(temp,'')
...
def down_page_run(urlstr):
    ...
    for num in range(0, thread_num - 1):
        downlist = Down_sdjnxs_Page(postid, url, num * numperthread + oldpage, (num + 1) * numperthread + oldpage,
                                  my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_sdjnxs_Page(postid, url, numperthread * (thread_num - 1) + oldpage, my_page + 1, my_page, oldrc,
                              resultdic )
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
...
if __name__ == '__main__':
    ...
    down_page_run("thread-38034-1-1.html")
