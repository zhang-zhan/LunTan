...
import time
class Down_RiZhao_Post(threading.Thread):
    ...
    def run(self):
        ...
        for num in range(self.startnum, self.endnum):
            ...
            self.pageurl = self.pageurl+"&page="+str(num)
            self.num = num
        ...
        for htstr in mydic:
            ...
            try:
                ...
                try:
                    ...
                    timeArray = time.strptime(rtime, "%Y-%m-%d %H:%M")
                except Exception as e:
                    try:
...
def down_page_run(urlstr):
    ...
    #设置线程数量
    thread_num = 4
    url = 'http://bbs.rznews.cn/'+ urlstr
    """print 'URL:%s' % url"""
    patterMatch = re.compile(r"tid=(\d+)")
    ...
    for num in range(0, thread_num-1):
        downlist = Down_RiZhao_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_RiZhao_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
if __name__ == '__main__':
    ...
    down_page_run("forum.php?mod=viewthread&tid=174905&extra=page%3D5")
