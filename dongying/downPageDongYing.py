...
import time
class Down_DongYing_Post(threading.Thread):
    ...
    def down_text(self, last_page):
        ...
        if((self.num == 1) and (self.rc == 0)):
            ...
            try:
                ctime = soup.find_all('td', class_='plc')[2].find("div",class_="authi").find("em").get_text()[4:]
...
def down_page_run(urlstr):
    ...
    #设置线程数量
    thread_num = 4
    url =  url = 'http://bbs.dongyingnews.cn/'+ urlstr
    """print 'URL:%s' % url"""
    strs = urlstr.split('-')
    postid = strs[1]
    #需要自己实现
    ...
    for num in range(0, thread_num-1):
        downlist = Down_DongYing_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_DongYing_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
if __name__ == '__main__':
    ...
    down_page_run("thread-1976389-1-17.html")
