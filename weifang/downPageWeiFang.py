...
import time
class Down_WeiFang_Post(threading.Thread):
...
def down_page_run(urlstr):
    ...
    for num in range(0, thread_num-1):
        downlist = Down_WeiFang_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_WeiFang_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
def insert_log(record):
    ...
    if os.path.exists("./log/") is False:
        os.mkdir("./log/")
    with open('log/downWeiFangData.log', 'a') as log:
...
if __name__ == '__main__':
    ...
    down_page_run("thread-550042-3-2.html")
