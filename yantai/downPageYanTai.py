...
import os
import time
class Down_YanTai_Post(threading.Thread):
    ...
    def down_text(self, last_page):
        ...
        if((self.num == 1) and (self.rc == 0)):
            ...
            auid = soup.find('div', class_='pi').find('div',class_="authi").find('a')["href"].split("&")[-1].split("=")[-1]
            try:
                ctime = soup.find_all('td', class_='plc')[2].find('div',class_="pi").find("div",class_="authi").find("em").get_text()[4:]
            except:
                ctime = soup.find_all('td', class_='plc')[4].find('div',class_="pi").find("div",class_="authi").find("em").get_text()[4:]
        ...
        for htstr in mydic:
            ...
            if((mi == 0) and (self.num == 1)):
                ...
                if(self.rc == 0):
                    ...
                    useLess = ""
                    try:
                        useletxt = soup2.find('td', class_="plc").find("td",class_="t_f").find_all("i",class_="pstatus")
                        for ins in useletxt:
                            try:
                                strtext = strtext.replace(ins.get_text(),"")
                            except Exception as e:
                                print (e)
                                continue
                    except Exception as e:
                    ...
                    imgsrc =""
                    try:
                        imgFile= soup2.find('td', class_='plc').find_all("ignore_js_op")
                        for img in imgFile:
                    ...
                    infoText = ""
                    try:
                        oinfoText = soup2.find('td', class_='plc').find_all("ignore_js_op")
            ...
            try:
                ...
                oinfo = ""
                try:
                    oinfo = soup2.find("td","plc").find_all("ignore_js_op")
                    for info in oinfo:
                        strtext = strtext.replace(info.get_text(),"")
                except Exception as e:
                        print (e)
                ...
                imgsrc =""
                try:
                    imginfo = soup2.find("td","plc").find_all("ignore_js_op")
                    for img in imginfo:
                        for im in img.find_all("img"):
                            try:
                                imgsrc += im["file"] + "\n"
                ...
                redic['un'] = uname
                uid = soup2.find('td', class_='pls').find('div',class_="authi").find("a")["href"].split("&")[-1].split("=")[-1]
                redic['uid'] = uid
                if(self.uid == uid):
                    redic['au'] = True
                else:
...
def down_page_run(urlstr):
    ...
    #设置线程数量
    thread_num = 4
    url = 'http://bbs.shm.com.cn/'+ urlstr
    strs = urlstr.split('-')
    postid = strs[1]
    #需要自己实现
    ...
    for num in range(0, thread_num-1):
        downlist = Down_YanTai_Post(postid, url, num*numperthread+oldpage, (num+1)*numperthread+oldpage, my_page, oldrc, resultdic)
        downlist.start()
        threads.append(downlist)
    downlist = Down_YanTai_Post(postid, url, numperthread*(thread_num-1)+oldpage, my_page+1, my_page, oldrc, resultdic)
    downlist.start()
    threads.append(downlist)
    """检查下载完成后再进行写入"""
    for t in threads:
...
def insert_log(record):
    ...
    if os.path.exists("./log/") is False:
        os.mkdir("./log/")
    with open('log/downYanTaiData.log', 'a') as log:
...
if __name__ == '__main__':
    ...
    down_page_run("thread-1090431-1-19.html")
